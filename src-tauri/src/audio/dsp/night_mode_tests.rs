...
mod tests {
    ...
    fn test_night_mode_peak_reduction() {
        ...
        let baseline_params = Arc::new(ArcSwap::from_pointee(DspParams::default()));

        let mut night_p = DspParams::default();
        apply_night_mode(&mut night_p, true);
        assert!(night_p.is_night_mode);
        assert_eq!(night_p.active_preset, "Night Mode");
        let night_params = Arc::new(ArcSwap::from_pointee(night_p));
        ...
        let mut baseline_chain = DspChain::new(sample_rate, channels, baseline_params);
        let mut night_chain = DspChain::new(sample_rate, channels, night_params);

        let mut baseline_buf = test_buffer.clone();
        let mut night_buf = test_buffer;

        baseline_chain.process_interleaved(&mut baseline_buf);
        night_chain.process_interleaved(&mut night_buf);
        ...
        assert!(
            night_max < 0.98,
            "Night mode peak should be constrained below ceiling, got {}",
            night_max
        );
        assert!(
